...
fn minimal_vmdk_virtual_disk_size() {
    let path = format!("{DATA_DIR}/minimal.vmdk");
    let reader = vmdk::VmdkReader::open(Path::new(&path)).expect("minimal.vmdk must open");
    assert_eq!(
...
fn minimal_vmdk_sector0_is_zeros() {
    let path = format!("{DATA_DIR}/minimal.vmdk");
    let mut reader = vmdk::VmdkReader::open(Path::new(&path)).expect("open");
...
fn dfvfs_ext2_vmdk_opens_and_has_nonzero_size() {
    let path = format!("{DATA_DIR}/dfvfs_ext2.vmdk");
    let reader = vmdk::VmdkReader::open(Path::new(&path)).expect("dfvfs_ext2.vmdk must open");
    assert!(
...
fn stream_opt_vmdk_returns_err() {
    let path = format!("{DATA_DIR}/stream_opt.vmdk");
    let result = vmdk::VmdkReader::open(Path::new(&path));
    assert!(
        result.is_err(),
